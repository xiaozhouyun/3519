...
#define QUESTION3_STOP_ODOMETER_MM    (1640.0f)
#define QUESTION3_DECEL_DISTANCE_MM   (200.0f)
#define QUESTION3_MIN_SPEED           (200)
...
static void ChassisTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        {
            int follow_speed = g_active_line_controller->base_speed;
            //慢减速
            if (FollowLine_Get_Active_Question() == 3U &&
                g_odometer_mm >= QUESTION3_STOP_ODOMETER_MM) {
                BT_Stop();
                MG513XGMR_Stop_All();
            } else {
                if (FollowLine_Get_Active_Question() == 3U &&
                    g_odometer_mm >= (QUESTION3_STOP_ODOMETER_MM - QUESTION3_DECEL_DISTANCE_MM)) {
                    float remaining_mm = QUESTION3_STOP_ODOMETER_MM - g_odometer_mm;
                    int min_speed = follow_speed;

                    if (min_speed > QUESTION3_MIN_SPEED) {
                        min_speed = QUESTION3_MIN_SPEED;
                    }

                    follow_speed = min_speed + (int)((follow_speed - min_speed)
                                   * remaining_mm / QUESTION3_DECEL_DISTANCE_MM);
                }

                FollowLine_Update(&g_grayscale_sensor, follow_speed);
                MG513XGMR_Update();
            }
        }
        else
...
int main(void)
{
    ...
    FollowLine_Init(&g_question4_line_controller, 4.20f, 0.0f, 0.002f, 600);
    FollowLine_Init(&g_question5_line_controller, 4.20f, 0.0f, 0.002f, 600);
