...
if ($followLine -notmatch 'LineController_t\s+g_line_controller\s*;') {
    throw 'g_line_controller must use zero initialization.'
}

if ($followLine -match '(?s)LineController_t\s+g_line_controller\s*=\s*\{') {
    throw 'Follow-line PID parameters must not have a second global default source.'
}
